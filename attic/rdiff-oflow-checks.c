...
void print_usage(const bool client, const bool daemon) {
  ...
  fprintf(stderr, "Usage: rdiff [-h] [c|d] ...\n\n");

  if (client) {
    fprintf(stderr, "Client usage: rdiff c [-46] -r DIR -a HOST -p PORT\n");
  ...
  if (daemon) {
    fprintf(stderr, "Daemon usage: rdiff d [-46dsk] -r DIR [-a HOST] -p PORT\n");
    fprintf(stderr, " -h       Show this help\n");
    fprintf(stderr, " -4       IPv4 only\n");
    fprintf(stderr, " -6       IPv6 only\n");
